...
import { getAllReviewers, approveReviewer, rejectReviewer, updateReviewer } from '../lib/supabase';
import { Button, Card, Badge, Input, Alert, Modal, PageHeader } from '../components/UI';
import { CheckCircle, X, Edit3, ChevronDown, ChevronUp, Shield } from 'lucide-react';
...
function ReviewerCard({ reviewer, onApprove, onReject, onEdit }) {
  ...
  return (
    <div className={`border rounded-2xl overflow-hidden ${reviewer.approvalStatus === 'pending' ? 'border-amber-200 bg-amber-50/30' : reviewer.approvalStatus === 'approved' ? 'border-emerald-200 bg-emerald-50/20' : 'border-red-200 bg-red-50/20'}`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 flex-wrap">
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge variant={STATUS_COLORS[reviewer.approvalStatus] || 'default'}>{reviewer.approvalStatus?.toUpperCase()}</Badge>
            ...
            <div className="flex gap-2 flex-shrink-0">
              ...
              {reviewer.approvalStatus === 'approved' && <Badge variant="success" size="sm"><CheckCircle size={11} className="mr-1" />Active</Badge>}
            </div>
          </div>
          ...
          {expanded && (
            <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
              {[['Role on Assignment', reviewer.role], ['Created', reviewer.createdAt?.substring(0, 10)]].filter(([, v]) => v).map(([k, v]) => (
