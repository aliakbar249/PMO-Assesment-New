...
export default function ReviewerAssessment({ onNavigate }) {
  ...
  const existingReview = useMemo(() => (reviewer && employee) ? getReview(reviewer.id, employee.id) : null, [reviewer, employee, currentUser]);

  // Steps = [...sections, 'assignments']
  const STEPS = [...sections.map(s => ({ type: 'section', ...s })), { type: 'assignments', id: 'assignments', title: 'Assignment Ratings' }];

  const [activeStep, setActiveStep] = useState(0);
  const [localSections, setLocalSections] = useState(() => existingReview?.sections || {});
  const [localAssignRatings, setLocalAssignRatings] = useState(() => existingReview?.assignmentRatings || {});
  const [saved, setSaved] = useState(false);
  const [submitModal, setSubmitModal] = useState(false);
  const [submitted, setSubmitted] = useState(existingReview?.status === 'submitted');
  ...
  const handleSave = () => {
    if (step.type === 'section') saveReviewProgress(reviewer.id, employee.id, step.id, localSections[step.id] || {});
    else saveAssignmentReview(reviewer.id, employee.id, localAssignRatings);
    refresh();
    setSaved(true);
  ...
  const allDone = sections.every(s => sectionDone(s.id));

  const handleSubmit = () => {
    sections.forEach(sec => saveReviewProgress(reviewer.id, employee.id, sec.id, localSections[sec.id] || {}));
    saveAssignmentReview(reviewer.id, employee.id, localAssignRatings);
    submitReview(reviewer.id, employee.id);
    refresh();
    setSubmitted(true);
    ...
    <div>
      ...
      <Card className="p-4 mb-5">
        ...
        <div className="flex gap-2 mt-3 flex-wrap">
          {STEPS.map((s, i) => (
            <button key={s.id} onClick={() => { handleSave(); setActiveStep(i); }}
      ...
      <div className="flex items-center justify-between gap-3 mt-4 flex-wrap">
        <div className="flex gap-2">
          <Button variant="secondary" size="sm" onClick={() => { handleSave(); setActiveStep(a => Math.max(0, a - 1)); }} disabled={activeStep === 0}>
            <ChevronLeft size={16} />Previous
          </Button>
          {activeStep < STEPS.length - 1 && (
            <Button size="sm" onClick={() => { handleSave(); setActiveStep(a => a + 1); }}>Next <ChevronRight size={16} /></Button>
          )}
        </div>
        <div className="flex gap-2 items-center">
          {saved && <span className="text-xs text-emerald-600 flex items-center gap-1"><CheckCircle size={12} />Saved</span>}
          <Button variant="secondary" size="sm" onClick={handleSave}><Save size={14} />Save</Button>
          ...
          <div className="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-amber-50 to-white">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">{assign.role} · {assign.organization}</p>
              </div>
            </div>
          </div>
